...
JSON payload keys are supplied in snake_case and recursively camelized before serialization. JSON responses are recursively snakized like SOAP responses. Per Microsoft's docs, JSON responses only ever expose `PartialErrors`/`NestedPartialErrors` (Campaign Management) or `Errors` (Bulk's `GetBulkUploadStatus`) — never SOAP's `BatchErrors`/`OperationErrors` — so JSON errors raise `Errors::PartialError`, `Errors::NestedPartialError`, or `Errors::AdApiFaultDetail`, the same SOAP classes used for those fields.
