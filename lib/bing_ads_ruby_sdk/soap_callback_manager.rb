...
class BingAdsRubySdk::SoapCallbackManager
  def self.register_callbacks
    ...
    BingAdsRubySdk.request_callback.for('hash_params.before_build') << lambda do |args, _node, type|
      before_build(args, type)
    end

    # Modify the response data whilst it is being processed by the SOAP client
    BingAdsRubySdk.response_callback.for('hash_builder.after_children_hash') << lambda do |hash, _node, _type|
